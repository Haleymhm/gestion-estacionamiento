...
export async function GET(request: NextRequest) {
  try {
    ...
    const [parkings, total] = await prisma.$transaction([
      prisma.parking.findMany({ skip, take }),
    ...
    console.error('GET /api/v1/parkings failed:', error);
...
export async function POST(request: Request) {
  try {
    ...
    const newParking = await prisma.parking.create({ data });
    return NextResponse.json(newParking, { status: 201 });
  } catch (error) {
    console.error('POST /api/v1/parkings failed:', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}